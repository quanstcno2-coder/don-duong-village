...
function render(items){
  ...
  items.forEach(p=>{
    ...
    <div class="product-info"><h3>${safe(p.name)}</h3><div class="price">${money(p.price)}</div><div class="actions"><a class="btn secondary small" href="product.html?id=${p.id}">Chi tiết</a><button class="btn primary small">+ Giỏ hàng</button></div></div>`;
